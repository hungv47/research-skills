---
name: experiment
description: "This skill should be invoked when the user has a business idea or initiative and wants to validate it with a test before fully committing. Triggers include 'how do I test this', 'should I run an A/B test', 'split test', 'how do I know if this will work', 'validate this idea with data', 'design an experiment', 'de-risk this launch', 'test our pricing', 'beta test', 'pilot program', 'soft launch', 'smoke test', or 'what's the cheapest way to learn' -- even if they just say 'should I try this and want to de-risk it with data', are about to launch something and want confidence it will work, or have targets set and need to test before full rollout. For testing a specific price or page variant with users, use this skill. For modeling whether pricing math works, use funnel-planner. Not for strategic prioritization (use solution-design), market sizing or feasibility research (use icp-research), setting targets (use funnel-planner), auditing an existing page's conversion (use lp-optimization), diagnosing why metrics are declining (use problem-analysis), or code testing, QA, or test suites."
